(function () {
  ...
  async function probeOpenCode(preferred) {
    ...
    if (!candidates.includes(LIVE_PROXY)) candidates.push(LIVE_PROXY);
  ...
  async function autoTitleFromPrompt(text) {
    ...
    const next = titleFromPrompt(text);
    if (!next) return;
    sess.title = next;
    saveSessions();
    renderSessions();
  ...
  async function sendPrompt(text) {
    ...
    await autoTitleFromPrompt(text);
    setThinking("run");
    ...
    try {
      ...
      const sess = activeSession();
      if (thinkAcc) sess.lines.push({ cls: "think", text: thinkAcc });
      if (acc) sess.lines.push({ cls: "assistant", text: acc });
      if (thinkAcc || acc) saveSessions();
      if (!acc) assistant.remove();
      if (!thinkAcc && thinkEl.isConnected) thinkEl.remove();
